...
*)
let rec obtenir_premiers_elements_contrainte_memoire l r m =
    if (r = 0 || m = 0) then
        []
    else
        match l with
        ...
        | t::q ->	let mem = Vertex.memory (V.label t) in
			if (mem > m) then
			    ...
			    t::(obtenir_premiers_elements_contrainte_memoire q (r-1) (m-mem))
;;
...
(*
 * Appliquer une itération de l'ordonnanceur avec heuristique et contrainte mémoire
 * INPUT :
 ...
 *)
 let rec iteration_ordonnanceur_contrainte_memoire dag y z r m =
    match y with
    | [] -> z
    | _ ->   let y_trie = List.fast_sort comparaison y in
                let liste_vi = obtenir_premiers_elements_contrainte_memoire y_trie r m in
                ...
                iteration_ordonnanceur_contrainte_memoire dag new_y new_z r m;
;;
...
let ordonnanceur_contrainte_memoire r m dag =
    ...
    iteration_ordonnanceur_contrainte_memoire dag (obtenir_sources dag) [] r m
;;
