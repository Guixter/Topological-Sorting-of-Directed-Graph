...
*)
let rec obtenir_premiers_elements l r =
    if (r = 0) then
        []
    else
        match l with
        | [] -> []
        | t::q -> t::(obtenir_premiers_elements q (r-1))
;;

(*
 * Enlever les premiers éléments d'une liste
 * INPUT :
 *  - l : la liste
 *  - r : le nombre de tâches souhaité ; si r est plus grand que la taille de l, on renvoie []
 *
 * OUTPUT :
 *  - la liste privée de ses r premiers éléments
*)
let rec enlever_premiers_elements l r =
    if (r = 0) then
        l
    else
        match l with
        | [] -> []
        | t::q -> enlever_premiers_elements q (r-1)
;;
...
(*
 * Appliquer une itération de l'ordonnanceur avec heuristique
 * INPUT :
 ...
 *)
 let rec iteration_ordonnanceur_avec_heuristique dag y z r =
    match y with
    | [] -> z
    | _ ->   let y_trie = List.fast_sort comparaison y in
                let liste_vi = obtenir_premiers_elements y_trie r in
                let y_moins = enlever_premiers_elements y_trie r in
                ...
                iteration_ordonnanceur_avec_heuristique dag new_y new_z r;
;;
...
let ordonnanceur_avec_heuristique r dag =
    ...
    iteration_ordonnanceur_avec_heuristique dag (obtenir_sources dag) [] r
;;
